...
export async function getAllTutorials(page = 1, sort = 'new', search = '') {
...
export async function getAllTutorialsByUser(user, page = 1, sort = 'new', search = '') {
  const { data, count } = await transformData(
...
export async function createTutorial(by, title, contents) {
  ...
  const id = tutorials.data.sort((a, b) => b.id - a.id)[0].id + 1;
